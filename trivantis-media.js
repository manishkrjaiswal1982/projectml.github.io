  ...
  else //If it not any of the cases above we can assume it is media element compatible
  {
	  ...
	  if (!window.HideMediaSpeedControl)
		...
		this.playerOptions = { 
				...
				tracksText:Varssbt_txt.getValue(), //Added custom text for accessibility
                tracksTextoff:Varhsbt_txt.getValue(), //Added custom text for accessibility
				success:function(mediaElem, domObj){
					...
					if(mediaElem.player){
							...
							mediaElem.player.captionsButton.addEventListener('click', function()
							{
								if ( THIS.mePlayer.player.selectedTrack == undefined )
								{
									if ( typeof (getDisplayWindow().VarTrivSelCap) == "object" )
										...
										//Added set caption button aria level toggle for accessibility           
										mediaElem.player.captionsButton.querySelectorAll('button')[0].setAttribute('title', mediaElem.player.options.tracksText);
										mediaElem.player.captionsButton.querySelectorAll('button')[0].setAttribute('aria-label', mediaElem.player.options.tracksText);
								}
								else
								{
									VarTrivSelCap.set(THIS.mePlayer.player.selectedTrack.srclang);
									mediaElem.player.captionsButton.querySelectorAll('button')[0].setAttribute('title', mediaElem.player.options.tracksTextoff);
									mediaElem.player.captionsButton.querySelectorAll('button')[0].setAttribute('aria-label', mediaElem.player.options.tracksTextoff);
								}
							})
						}
						...
						mediaElem.player.enterFullScreen = function(){
							...
							if (iOS14Fix.isNeeded())	// LD-8137
								iOS14Fix.applyiOS14Fix(THIS);
							...
							THIS.isFullScreen = true;
							 //Edit for Exit fullscreen message
							 this.fullscreenBtn.firstChild.setAttribute("title",VarEFS_txt.getValue())
							 this.fullscreenBtn.firstChild.setAttribute("aria-label",VarEFS_txt.getValue())
							if(!is.iOS && !is.safari){
						...
						mediaElem.player.exitFullScreen = function(){
							if(THIS.isFullScreen)
							{
								...
								//Edit for Exit fullscreen message
								this.fullscreenBtn.firstChild.setAttribute("title",VarFS_txt.getValue())
								this.fullscreenBtn.firstChild.setAttribute("aria-label",VarFS_txt.getValue())
...
// LD-8137. Caption fix for iOS < 15.4
// Set track.cue[i].line = -3. This allows 2 lines of text.
iOS14Fix = {
	minMajor: 15,
	minMinor: 4,
	cueLine: -3,
	isNeeded: function ()
	{
		// This fix is needed for iOS version < 15.4 and iOS Chrome
		var safariResult = navigator.userAgent.match(/(iPhone|iPad).*Version\/(\d*)\.(\d*)/),
			chromeResult = navigator.userAgent.match(/(iPhone|iPad).*CriOS/),
			bNeeded = false;
		
		// detection for newer iPads. It has "Macintosh", not "iPad"
		if (!safariResult)
			safariResult = navigator.maxTouchPoints && navigator.maxTouchPoints > 2 ? navigator.userAgent.match(/(Macintosh).*Version\/(\d*)\.(\d*)/) : null;
		
		if (is.iOS)
		{
			if (safariResult && safariResult.length > 3)
			{
				var major = parseInt(safariResult[2]),
					minor = parseInt(safariResult[3]);
				if (!isNaN(major) && !isNaN(minor) && (major < iOS14Fix.minMajor || major == iOS14Fix.minMajor && minor < iOS14Fix.minMinor))
				{
					bNeeded = true;
					console.log('iOS14Fix is needed for Safari and will be applied.');
				}
			}
			else if (chromeResult)
			{
				bNeeded = true;
				console.log('iOS14Fix is needed for Chrome and will be applied.');
			}
		}
		return bNeeded;
	},
	applyiOS14Fix: function(objMedia) {
		var vidElem = objMedia.mediaPlayer;
		for ( var i = 0; i < vidElem.textTracks.length; i++)
		{
			// cue.line is set to "auto" in iOS >= 15, but lower versions do not
			// allow strings. 
			var track = vidElem.textTracks[i];
			for (var j = 0; j < track.cues.length; j++)
				track.cues[j].line = iOS14Fix.cueLine;
		}
	}
}
