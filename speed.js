...
mejs.i18n.en['mejs.speed-rate'] = 'Speed Rate1';
...
Object.assign(MediaElementPlayer.prototype, {
	buildspeed: function buildspeed(player, controls, layers, media) {
		...
		var speeds = [],
			...
			speedTitle = mejs.Utils.isString(t.options.speedText) ? t.options.speedText : VarSpeedTxt.getValue(),
		...
		for (var _i2 = 0, _total2 = inEvents.length; _i2 < _total2; _i2++) {
			player.speedButton.addEventListener(inEvents[_i2], function () {
				...
				if (triv$('video', player.container).length || (player.speedSelector.offsetHeight < triv$(player.container).offset().top))	// LD-7450
		...
		for (var _i4 = 0, _total4 = radios.length; _i4 < _total4; _i4++) {
			...
			radio.addEventListener('click', function () {
				...
				// LD-7445
				if (t.options.speedUpdated) {
					t.options.speedUpdated(newSpeed);
                }
			});
		}
		...
		player.speedSelector.parentNode.addEventListener('keydown', function (e) {
			switch (e.keyCode) {
				case 38:
					for (var x = 1; x < radios.length; x++) {
						if (radios[x].checked) {
							var prevRadio = radios[x - 1];
							prevRadio.dispatchEvent(mejs.Utils.createEvent('click', prevRadio));
							break;
						}
					}
				...
				case 13:
				case 27:
					triv$('button', player.speedButton).blur();
			}
			e.stopPropagation();
		});
