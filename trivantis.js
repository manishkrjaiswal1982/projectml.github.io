...
function isInFrame(wndow, count)
{
	if (wndow.frameElement && wndow.frameElement.tagName.toLowerCase() == 'frame')
		...
		return isInFrame(wndow.parent, ++count);
	return false;
}
...
var setFixedTimeout=0;
function setfixed(){
	if(setFixedTimeout==0){
		setfixed_cust();
	}else{
	clearTimeout(setFixedTimeout); 
	setFixedTimeout=setTimeout(setfixed_cust, 500);
	}
}
function setfixed_cust()
{
...
//window.bFitToWidth=true;
 ...
 // 	bForceShrunk = true;
	...
	scaleFill= scaleFill*calcZooming()
	scaleFill=scaleFill>1?1:scaleFill;
	...
	if (window.bFitToMobile && is.isMobile.any())
	{
		...
		transX = 0, transY = 0;
console.log("scaleFill 1: "+scaleFill)
console.log("transX : "+transX)
console.log("transY : "+transY)
console.log("bForceShrunk : "+bForceShrunk)
console.log("scaleOffset : "+scaleOffset)
	
/*if(windowW > iScaleMaxWidth*scaleFill && iScaleMaxWidth != 0)
	{
		bForceShrunk = true;
		//windowW = iScaleMaxWidth;
	}*/
...
if(!is.isMobile.any()){
	if(scaleFill==1 && calcZooming()==1){
		console.log("full scale......................")
		setTimeout(function(){
			var pgElem=$("#pageDIV");
			$(".DLG_window").css({"width":"100%","height":"100%","transform":"scale(1)","left":"0px"})},20);
	...
	//	setTimeout(function(){
		//	var pgElem=$("#pageDIV");
		//	$(".DLG_window").css({"width":pgElem.css("width"),"height":pgElem.css("height"),"transform":pgElem.css("transform"),"left":pgElem.css("left")})},20);
	}
...
function calcZooming(){
	...
	if(is.Mac){
		 var newPx_ratio = window.screen.availWidth / document.documentElement.clientWidth;
